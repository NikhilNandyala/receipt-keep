...
function saveData(receipts) {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(receipts)); }
  catch { console.warn("Storage full or unavailable."); }
}
...
async function scanReceiptImage(base64, mediaType) {
  ...
  if (!apiKey) {
    throw new Error("API key missing. Add VITE_ANTHROPIC_API_KEY to your .env.local file.");
  }
    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          ...
          {
            type: "text",
            text: `Look at this receipt and extract the details. Reply ONLY with a JSON object — no markdown, no explanation.

{
  "store": "merchant or store name",
  "amount": total as a number e.g. 12.50,
  ...
  "notes": "brief summary of items if readable, else empty string"
}`,
          },
        ],
      }],
...
.install-banner button { background: #f5f0e8; color: #1a1a1a; border: none; border-radius: 3px; padding: 8px 14px; font-family: var(--font-sans); font-size: 13px; font-weight: 600; cursor: pointer; white-space: nowrap; }
.install-banner .dismiss { background: none; color: #888; padding: 4px; font-size: 18px; }
`;
...
export default function App() {
  ...
  const [scanStatus, setScanStatus] = useState(null); // 'ok' | 'err'
  ...
  const byCategory = CATEGORIES.map(cat => ({
    cat,
    total: receipts.filter(r => r.category === cat).reduce((s, r) => s + r.amount, 0),
        ...
        {showInstall && (
          <div className="install-banner">
            ...
            <div className="install-banner-text">
              <strong>Install ReceiptKeep</strong><br/>Add to your home screen for quick access
            </div>
            <button onClick={handleInstall}>Install</button>
            <button className="btn dismiss" onClick={() => setShowInstall(false)}>✕</button>
          </div>
        )}
              ...
              <div className="card" style={{padding:16, marginBottom:16}}>
                ...
                {byCategory.map(({ cat, total, count }) => (
                  <div className="cat-row" key={cat}>
                    ...
                    <div className="bar">
                      <div className="bar-fill" style={{ width: `${(total / totalAll) * 100}%`, background: CATEGORY_COLORS[cat] }} />
                    </div>
            ...
            <div className="card" style={{padding:"20px 16px"}}>
              ...
              <div style={{display:"flex", flexDirection:"column", gap:16}}>
                <div className="field">
                  ...
                  <input className={`input${errors.store ? " err" : ""}`} placeholder="e.g. Tesco, Amazon…"
                ...
                <div className="field">
                  ...
                  <textarea className="input" placeholder="Add any notes…" rows={3}
    ...
    <div className="receipt-row">
      ...
      <div className="row-info">
        ...
        <div className="row-meta">
          ...
          <span className="tag" style={{background: CATEGORY_COLORS[r.category] + "22", color: CATEGORY_COLORS[r.category]}}>
            {r.category}
          </span>
        </div>
